    ...
    <div className="dash min-h-screen">
      <header className="border-b border-slate-800/80 px-6 py-4">
        <div className="mx-auto flex max-w-5xl items-center gap-3">
          <Link href="/app" className="text-xs text-slate-400 transition hover:text-slate-200">
            ← Dashboard
          </Link>
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-6 py-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
          {[
            ...
            <div key={k.label} className="rounded-xl border border-slate-800 bg-[#0d1420] p-3">
              ...
              <div className="mt-1 text-lg font-semibold text-slate-100">{k.value}</div>
            </div>
          ))}
        </div>
        ...
        {anomalies.length > 0 && (
          <div className="space-y-2">
            {anomalies.map((a) => (
              <div
                key={a.id}
                className="rounded-xl border border-amber-900/50 bg-[#0d1420] p-3 text-xs"
              >
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <section className="rounded-xl border border-slate-800 bg-[#0d1420] p-4">
        ...
        {campaign.adSets.map((s) => (
          <section key={s.id} className="rounded-xl border border-slate-800 bg-[#0d1420]">
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody>
                  {s.ads.map((ad) => {
                    ...
                    return (
                      <tr key={ad.id} className="border-b border-slate-800/60 text-slate-300 last:border-0">
