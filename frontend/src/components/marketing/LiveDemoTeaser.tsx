        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <p className="text-lg text-[#78716C] max-w-md mb-8 leading-relaxed">
              Click the compressor to simulate a bearing failure. Watch as AI agents detect, diagnose, and generate a repair plan — automatically.
            </p>
            <Button
              asChild
              size="lg"
          ...
          >
            <div className="rounded-2xl bg-[#0C1018] p-6 border border-white/[0.06]">
              {/* Health dial */}
              <div className="flex items-center justify-center mb-5">
                <button
                  onClick={triggerDemo}
                  className={`relative w-28 h-28 ${!triggered ? 'cursor-pointer group' : 'cursor-default'}`}
                  disabled={triggered}
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-[8px] text-white/25 font-mono">COMP-002</span>
                  </div>
                  {/* Click hint */}
                  {!triggered && (
                    <motion.div
                      className="absolute -bottom-1 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-full bg-[#C4A77D]/20 border border-[#C4A77D]/30"
                      animate={{ scale: [1, 1.05, 1] }}
                      transition={{ duration: 1.5, repeat: Infinity }}
                    >
                      <span className="text-[8px] text-[#C4A77D] font-semibold">Click to break</span>
                    </motion.div>
                  )}
                </button>
              </div>
                ...
                <AnimatePresence>
                  {responseCards.map((card) => {
                    ...
                    return (
                      <motion.div
                        key={card.title}
                        className={`flex items-start gap-3 rounded-lg ${card.bgColor} border ${card.borderColor} px-3 py-2.5`}
                      ...
                      >
                        ...
                        >
                          <CardIcon className="size-3.5" style={{ color: card.color }} />
                        </div>
                        <div>
                          <p className="text-[10px] font-semibold text-white/60">{card.title}</p>
                          <p className="text-[8px] text-white/25 font-mono">{card.detail}</p>
                        </div>
                      </motion.div>
                    );
                  })}
              ...
              <AnimatePresence>
                {phase >= 5 && (
                  <motion.button
                    className="flex items-center gap-1.5 mx-auto mt-4 px-3 py-1.5 rounded-md bg-white/[0.05] border border-white/[0.08] text-[9px] text-white/40 hover:text-white/60 hover:border-white/[0.12] transition-colors"
